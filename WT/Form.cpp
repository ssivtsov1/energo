...
TMenuItem *TWTForm::ClientMenuItem;      // указатель на меню исходных документов
TMenuItem *TWTForm::InDocMenuItem;      // указатель на меню исходных документов
TMenuItem *TWTForm::CalcMenuItem;     // указатель на меню выходных документов
...
TMenuItem *TWTForm::ToolsMenuItem;
...
_fastcall TWTMDIWindow::~TWTMDIWindow() {
  ...
  if (ID!="") {
    ...
    ((TWTMainForm*)MainForm)->StartupIniFile->WriteInteger(ID,"WindowState",WindowState);
  }
...
void __fastcall TWTMDIWindow::FSetID(AnsiString Value){
  ...
  int L1=10;
  int T1=30;
  int W1=Screen->Width-10;
  int H1=Screen->Height-30;
  int S1=wsNormal;
  //восстанавливаем координаты окна
  if (((TWTMainForm*)MainForm)->StartupIniFile->SectionExists(ID)) {
    L1=((TWTMainForm*)MainForm)->StartupIniFile->ReadInteger(ID,"Left",10);
    T1=((TWTMainForm*)MainForm)->StartupIniFile->ReadInteger(ID,"Top",30);
    W1=((TWTMainForm*)MainForm)->StartupIniFile->ReadInteger(ID,"Width",W1);
    H1=((TWTMainForm*)MainForm)->StartupIniFile->ReadInteger(ID,"Height",H1);
    S1=((TWTMainForm*)MainForm)->StartupIniFile->ReadInteger("Data","WindowState",wsNormal);
      if (L1<0 ) L1=0;
      if (T1<0 ) T1=0;
    if ((L1+W1) > Screen->Width)
       W1=Screen->Width-L1;
       if (W1<0) W1=100;
    if ((H1+T1) > Screen->Height)
       H1=Screen->Height-T1;
       if (H1<0) H1=100;
     if ((L1+W1) > Screen->Width)
       { L1=Screen->Width-W1;
...
void __fastcall TWTMDIWindow::ShowAs(AnsiString ID,int flag_modal){
