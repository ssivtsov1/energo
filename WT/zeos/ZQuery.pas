...
{$IFDEF VER150ABOVE}{$IFNDEF VERCLX}Variants, FmtBcd,{$ENDIF}{$ENDIF}
...
{$ENDIF}
    procedure SqlFilterRefresh; // Moved to public by PFAF
  protected
  ...
  public
    ...
    //Speed up the update process when parameters of a query change
    //without changing the return columns format.
    //procedure SqlFilterRefresh; //Moved to public by PFAF
    procedure SqlFilterRefreshEx(bForce:boolean); //Created by PFAF
    function  RefreshCurrRow :boolean; //Created by PFAF
...
{$IFNDEF NO_GUI}
{$IFNDEF LINUX},Forms, Controls {$ELSE} ,QForms, QControls
{$ENDIF}{$ENDIF};
...
{$ELSE}
  //Correction for PostgreSQL v7.2
  FOptions := [doHourGlass, doAutoFillDefs];
{$ENDIF}
...
begin
  ...
  FreeAndNil(FSqlBuffer);

  FreeAndNil(FIndexDefs);
  FreeAndNil(FParser);
  FreeAndNil(FCCParser);
...
{$ENDIF}
    ...
    while (not Query.EOF) and ((doQueryAllRecords in Options)
      or (Query.RecNo < MIN_FETCH_ROWS) or Force or FetchAll) do
      QueryRecord;
    { Invoke on progress event }
...
var
  ...
  i: integer; //mjd 10/5/2001 - added for trimming
begin
  Result := False;
...
{$ENDIF}
  ...
  else // field is not BCD and buffer is not null
  begin
    Result := SqlBuffer.GetFieldData(FieldDesc, Buffer, RecBuffer);
    // start mjd 9/21/2001 - to fix "trimming" bug #463857
    if (Buffer <> nil) and (Field.DataType = ftString) and (DatabaseType in [dtMySQL,dtMSSQL] ) then
    begin
      i := field.size-1;
      while (i >= 0) and (pchar(buffer)[i] = ' ') do
        dec(i);
      if (i < (field.size-1)) then
        pchar(buffer)[i+1] := #0;
    end;
  end;
end;
...
begin
  ...
  Result := Active;
  if Assigned(Query) then Result:=Result or Query.Active;
  if Assigned(SqlBuffer) then Result:=Result or (SqlBuffer.Count > 0);
end;
...
begin
  ...
  begin
    ...
    FRowsAffected := TransactObj.ExecSql(ConvertToSqlEnc(SqlParser.Text), self);
  end else
...
begin
  UpdateCursorPos;
  if (CurRec = -1) and (SqlBuffer.Count > 0) then
    Result := 1
  else
    Result := CurRec + 1;
end;
...
function TZDataset.RefreshCurrRow :boolean; //Created by PFAF
begin
Result:=RefreshCurrentRow(SqlBuffer.Items[CurRec]);
end;
...
begin
 ...
 // mjd 10/5/2001 - bug #463877 added for loop to allow refresh on multiple table queries
//  for j := 0 to sqlparser.Tables.Count - 1 do  // fix for bug #463877 is a work in progress - had to remove "fix" for now
  j := 0;
  begin
    Where := Trim(FormSqlWhere(SqlParser.Tables[j], RecordData));
    if Where = '' then Exit;
    Sql := 'SELECT * FROM ' + ProcessIdent(SqlParser.Tables[j]) + ' ' + Where;
    ...
    begin
      FieldDesc := SqlBuffer.SqlFields[I];
      tmp := query.FieldIndex(FieldDesc.field);
      if tmp < 0 then
      begin
        continue;
      end;
      if FieldDesc.FieldNo < 0 then Continue;
      IsNull := Query.FieldIsNull(tmp);
      ...
      else if IsNull and SQLBuffer.GetFieldNull(FieldDesc,RecordData) then       // mjd 10/5/2001 - avoid setting a null if both fields are supposed to be null
        continue
      else   //refresh only for no blob field
      if FieldDesc.BlobType = btInternal then
      begin
        FieldValue := Query.Field(tmp);
...
begin
  ...
  begin
    ...
    try
      ...
      { Restore position }  //Change suggested by Dmitry Chernikov on 11/13/2001
      if (not Value) and (not (doSqlFilter in Options)) then
...
begin
SqlFilterRefreshEx(False);
end;

procedure TZDataset.SqlFilterRefreshEx(bForce :boolean);
begin
  if (((not (State in [dsInactive])) and
       (ConvertToSqlEnc(SqlParser.Text) <> Query.Sql)) or bForce) then
  begin
    Query.Sql := ConvertToSqlEnc(SqlParser.Text);
    ShortRefresh;
    First;
...
begin
  ...
  if AComponent = FUpdateObject then
    FUpdateObject := nil;
  if assigned(FMasterLink) and (AComponent = FMasterLink.Datasource) then
  ...
  *)
  ...
  begin
    if RefreshCurrentRow(NewData) then
      ;
    (*ELITE: why this? In TDataSet.Post (Unit DB) Resync
      in called Resync([]);    // added by mjd 10/5/2001 for bug 463862
     *)
  end;
end;
...
begin
  ...
  case DatabaseType of
    dtPostgreSql:
      if (Value[1] <> '"') then     // and (Pos(' ', Result) > 0) removed by Panayiotis Fafakos
        Result := '"' + Value + '"';
    dtMsSql:
      if (Value[1] <> '[') then     // and (Pos(' ', Result) > 0) removed by WebHSW
        Result := '[' + Value + ']';
  end;
end;
  ...
  try
    ...
    if not Deleted then
      ...
      begin
        ...
        if OldRecCount = SqlBuffer.Count then
          Next;
      end
    else
...
begin
    ...
    ztModified:
      begin
        ...
        begin
          ...
          // start mjd 9/21/2001 - to fix "trimming" bug #463857
          if (DatabaseType in [dtMySQL,dtMSSQL] ) then
          begin
             if TrimRight(FieldValue) = TrimRight(SqlBuffer.GetField(FieldDesc, OldData)) then
               Continue;
          end
          else
   	   if (DatabaseType in [dtInterbase] ) then begin   
               FieldValue := StringReplace(SqlBuffer.GetField(FieldDesc,NewData),'''', '''''',   
                                           [rfReplaceAll]);   
           end   
           else begin 
          ...
          if (DatabaseType in [dtInterbase, dtDb2]) and
            ...
            and not SqlBuffer.GetFieldNull(FieldDesc, NewData)
  ...
  begin
    if ParamCount = 0 then
      AffectedRowCount := TransactObj.ExecSql(Sql, self)
    else
...
// itSDS Feldnamen als Überschrift ausgeben !!
  if SqlBuffer.Count > 0 then begin
    Buffer := '';
  ...
  for I := 0 to SqlBuffer.Count-1 do begin
    if not CheckRecordByFilter(I) then 
      Continue;
    RecordData := SqlBuffer[I];
    Buffer := #13#10;
    if I > 0 then Buffer := Buffer + #13#10;
    for J := 0 to SqlBuffer.SqlFields.Count-1 do begin
...
begin
  ...
  try
    while Stream.Position < Stream.Size do begin
      ...
      { Fill record buffer }
      for I := 0 to SqlBuffer.SqlFields.Count-1 do begin
        FieldDesc := SqlBuffer.SqlFields[I];
        Value := Trim(StrTokEx(Buffer, ',;'));
        if (Value <> '') and not StrCaseCmp(Value, 'NULL') then begin
          if Value[1] = '''' then
          begin
            DeleteQuotes(Value);
