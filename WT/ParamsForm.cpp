...
TWTParamItem* TWTParamsList::AddDb(AnsiString Label,TZDataset* DataSet,AnsiString FieldName,int EditLen,bool Break){
...
/*
TWTParamItem* TWTParamsList::AddButton(TWTPanel *Panel,TButton* Button, bool Break){
    TWTParamItem* Prop=new TWTParamItem(NULL);
  if (!Prop->Panel)
...
TWTParamItem* TWTParamsList::AddDate(AnsiString Label,int EditLen,bool Break){
  TWTParamItem* PI=AddEditParam(Label,EditLen,"",Break);
  ((TMaskEdit*)PI->FControl)->EditMask="99.99.9999";
...
TWTParamItem* TWTParamsList::AddButtonParam(TButton *Btn,bool Break){
  TWTParamItem* Prop=new TWTParamItem(NULL);
  if (Count) Prop->Break=Break;
      else Break=true;
  TButton* Btn1= new TButton(Prop);
  Btn1->Caption=Btn->Caption;
  Btn1->OnClick=Btn->OnClick;
  Btn1->Parent=
  Prop->FControl=Btn1;
  Prop->FParamsList=this;
  return Prop;
...
TWTParamItem* TWTParamsList::AddButton(TButton *Btn,bool Break){
   TWTParamItem* Prop=new TWTParamItem(NULL);
     if (Count) Prop->Break=Break;
      else Break=true;

  if (!Prop->Panel) Prop->Panel=new TPanel(Prop);
  Btn->Parent=Prop->Panel;
  Prop->Panel->Width=Btn->Width+2;
  Btn->Align=alClient;
  Prop->FControl=Btn;
  Prop->Kind=pkBtn;
  InsertToPanel(Prop);
  ...
  if (Param->Kind==pkSimple || Param->Kind==pkDate || Param->Kind==pkTime || Param->Kind==pkMask || Param->Kind==pkDb) {
    if (Param->Label->Width>LabelLen && Param->Break) {
      ...
      for (int x=0;x<Count;x++) {
        P=Get(x);
        if (!(P->Kind==pkSimple || P->Kind==pkDate || P->Kind==pkTime || P->Kind==pkMask || P->Kind==pkBtn)) continue;
...
/*  if (Edit) Edit->Text=Text;
  ...
  if (Kind==pkSimple || Kind==pkTime || Kind==pkMask) {
    ((TMaskEdit*)FControl)->Text=Text;
  }
  if (Kind==pkDate) {
   TDateTime T=StrToDate(Text);
   AnsiString SS=DateToStr(T);
    ((TMaskEdit*)FControl)->Text=SS;
  }
...
/*  if (Edit) return Edit->Text;
  ...
  if (Kind==pkDate) {
    AnsiString SS=((TMaskEdit*)FControl)->Text;
     TDateTime T=StrToDate(SS);
     SS=DateToStr(T);
     return SS;

  /*TDateTime T=StrToDate(Text);
   AnsyString SS=DateToStr(T);
    ((TMaskEdit*)FControl)->Text=SS; */
  }
...
void TWTParamItem::SetRequired(TZDataset* DataSet,AnsiString FieldName){
  if (Kind==pkSimple || Kind==pkDate || Kind==pkTime || Kind==pkMask) {
...
__fastcall TWTDoc::TWTDoc(Classes::TComponent* AOwner,AnsiString FName) :TWTMDIWindow(AOwner) {
  ...
  GridTabSheet->Caption=NULL;//"Печатная форма";
  MainTabSheet->Caption="";
  PageControl->TabHeight=1;
  PageControl->TabWidth=1;
  ...
  for (int y=0;y<Panel->Params->Count;y++) {
    if (Panel->Params->Get(y)->Kind==pkGrid) {
     ...
     // Grid->Adapter->AddFunction("Field","TDBGrid",Pointer);
          Grid->Adapter->AddGridDec(Panel->Params->Get(y)->ID,"string",&Panel->Params->Get(y)->TempValue);
