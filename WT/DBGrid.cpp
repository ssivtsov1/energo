...
void __fastcall TWTDBGrid::PutHandles() {
  ...
  if (Query) {
     ...
     Query->Options  << doQuickOpen;
     Query->Options  <<   doAutoFillDefs;
    // Query->Options  <<   doRefreshAfterPost;
     Query->Options  <<   doSqlFilter;
      //Query->LinkOptions << loLinkRequery;
      //Query->LinkOptions <<  loAlwaysResync;
...
void __fastcall TWTDBGrid::FOnDSStateChange(TObject *Sender){
  if (DataSource->State==dsInsert && InsertFlag) {
    //DataSource->DataSet->Cancel();   //2702  --
    //InsertFlag=true;                 //2702  --
  } else {
    InsertFlag=false;
  }
}
...
//    SetFilter();
    if (Table) {
     ...
     if (Query->BookmarkValid(SavePlace)) {
