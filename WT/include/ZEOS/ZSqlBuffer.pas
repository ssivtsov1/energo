...
{$ENDIF}
    ...
    //start merkuriev 24/12/2001 - to fix err #463917
    //{old line} RecordData.Bytes[FieldDesc.Offset] := 0;
    if ((FieldDesc.FieldType <> ftDate ) and (FieldDesc.FieldType <> ftDateTime))
        or (((FieldDesc.FieldType = ftDate) or (FieldDesc.FieldType = ftDateTime))
            and (Value <> 0)) then RecordData.Bytes[FieldDesc.Offset] := 0;
