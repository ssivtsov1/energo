...
{$ENDIF}
  ...
  begin
    ...
    case SqlVar.DataType of
      ...
      SQL_CLOB, SQL_DBCLOB, SQL_LONGVARCHAR, SQL_WLONGVARCHAR:
        SqlVar.ColType := ftMemo;
      SQL_BLOB:
...
begin
  ...
  case SqlVar.ColType of
    ...
    ftMemo, ftBlob:
      begin
        if (SqlVar.DataType = SQL_WLONGVARCHAR) or
          (SqlVar.DataType = SQL_LONGVARCHAR) then
...
begin
  ...
  else if StrCaseCmp(Value, 'LONG VARCHAR') then
  begin
    Result := ftMemo;
    BlobType := btInternal;
  end
