...
{ Convert MySql field types to delphi field types }
function MySqlToDelphiTypeDesc(Value: string; var Size, Precision: Integer; AlternativeView: Boolean): TFieldType;
...
//    if (Temp = 'SELECT') or (Temp = 'SHOW') or (Temp = 'DESCRIBE') then  // mjd 10/5/2001 - for bug #463872 mysql backup statement
    if (Temp = 'SELECT') or (Temp = 'SHOW') or (Temp = 'DESCRIBE') or (Temp = 'BACKUP') then  // mjd 10/5/2001 - for bug #463872 mysql backup statement
    begin
...
{ Convert MySql field types to delphi field types }
function MySqlToDelphiTypeDesc(Value: string; var Size, Precision: Integer;
           AlternativeView: Boolean): TFieldType;
begin
  Size := 0;
  Precision := 0;
  ...
  if AlternativeView then begin
   if (LowerCase(Value) = 'enum(''y'',''n'')')
     ...
     Result := ftBoolean
   end else if StrCmpBegin(Value,'enum') then
  begin
    Result := ftString;
