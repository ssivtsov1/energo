...
type
  ...
  public
    ...
    property NewStyleTransactions: Boolean read FNewStyleTransactions write FNewStyleTransactions;
  end;
...
//  StartTransaction; //old variant
  if (not FNewStyleTransactions) then StartTransaction; // instead simply "StartTransaction;"
...
begin
  ...
  begin
    ...
    if (not FNewStyleTransactions) then
    begin
     Result := PQexec(Handle, 'BEGIN');
     PQclear(Result);
     MonitorList.InvokeEvent('BEGIN', Error, Error <> '');
    end;
    if Error <> ''  then  SetStatus(csFail);
  end;
end;
...
begin
  if Active then Close;
  Sql := 'SELECT i.indexrelid AS index, t1.relname AS name, t2.relname AS table,'
