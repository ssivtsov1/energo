...
// mjd - 10/4/2001 - made ZConnect more "D5 friendly" by moving IFDEF Windows to IFNDEF Linux
{$IFNDEF LINUX}
{$INCLUDE ..\ZeosDef.inc}
{$ELSE}
{$INCLUDE ../ZeosDef.inc}
...
uses
  SysUtils, {$IFNDEF LINUX}Windows,{$ENDIF} Classes, ZToken, ZDirSql, ZConvert;
...
begin
  inherited Destroy;
  Disconnect;
  While FTransacts.Count>0 do begin
  	TZTransact(FTransacts[FTransacts.Count -1]).
     DataBase:=Nil;
  end;
  FDatasets.Free;
  FTransacts.Free;
