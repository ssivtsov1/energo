...
{$IFNDEF LINUX}
  ...
  Dialogs, Forms, Graphics, Db,
{$ELSE}
  QStdCtrls, QControls, QExtCtrls,
  QDialogs, QForms, QGraphics, DB,
{$ENDIF}
{$IFNDEF VERCLX}
  DsgnIntf,
{$ELSE}
...
{ Form constructor }
constructor TfrmLinkFields.Create(AOwner: TComponent);
begin
...
{ Form destructor }
destructor TfrmLinkFields.Destroy;
begin
  FJMaster.Free;
  FJDetail.Free;
  FMaster.Free;
...
var
  frmLinkFields : TfrmLinkFields;
begin
  frmLinkFields := TfrmLinkFields.Create(Application);
  with frmLinkFields do
    try
