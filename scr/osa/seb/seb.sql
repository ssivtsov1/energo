...
insert into syi_sysvars_tbl (id,ident,type_ident,value_ident)
  values (301,'path_exp','varchar','/home/local/seb/');

insert into syi_sysvars_tbl (id,ident,type_ident,value_ident)
  values (302,'flag_exp','varchar','1');

insert into syi_sysvars_tbl (id,ident,type_ident,value_ident)
  values (303,'flag_chmeter','varchar','1');

insert into syi_sysvars_tbl (id,ident,type_ident,value_ident)
  values (304,'flag_chlimit','varchar','1');

insert into syi_sysvars_tbl (id,ident,type_ident,value_ident)
  values (305,'flag_chtax','varchar','1');

insert into syi_sysvars_tbl (id,ident,type_ident,value_ident)
  values (306,'flag_dneprbill','varchar','1');

create or replace function upd_dnepr_flag() returns int  as
$BODY$
declare 
pr boolean;
kodres int;
begin
select value_ident::int into kodres from syi_sysvars_tbl where ident='kod_res'; 
if kodres>=100 then
 update syi_sysvars_tbl set value_ident=0 where ident= 'flag_dneprbill';
else
 update syi_sysvars_tbl set value_ident=1 where ident= 'flag_dneprbill';
end if;
...
alter table seb_obr_all_tbl add column   deb_pm18v  numeric(16,2);
alter table seb_obr_all_tbl add column   deb_pm18e  numeric(16,2);
alter table seb_obr_all_tbl add column   deb_pm18pdv  numeric(16,2);
alter table seb_obr_all_tbl add column   deb_km18v  numeric(16,2);
alter table seb_obr_all_tbl add column   deb_km18e  numeric(16,2); 
alter table seb_obr_all_tbl add column   deb_km18pdv  numeric(16,2); 
...
begin
 ...
 if found then  
  ...
  select b.mmgg as mmgg,c.kind_dep as cod_r,c.code as osob_r,c.id as id_client,b.id_pref,
    ...
    as d_doc,
   case when (b.idk_doc=200) then ''яВЕР''::varchar else ''яВЕР''::varchar end as vid_doc, 
    case when (b.id_pref=10 or b.id_pref=1099) then ''AE''::varchar else 
     case when (b.id_pref=20 or b.id_pref=2099) then ''PE''::varchar else 
      case when (b.id_pref=901) then ''оЕМЪ''::varchar else 
       case when (b.id_pref=902) then ''хМТК''::varchar else 
...
raise notice ''insert into seb_opl_tmp'';
  ...
  select p.mmgg as mmgg,c.kind_dep as cod_r,c.code as osob_r,c.id as id_client,
     p.reg_num as  nom_doc,
      p.reg_date as d_doc,
      p.id_pref, 
      case when (p.idk_doc=100 or p.idk_doc=110 or p.idk_doc=109) then ''аЮМЙ'' else
       case when p.idk_doc=120 then ''юБiГН'' else
        case when p.idk_doc=199 then ''йНПХЦСБЮММЪ'' else ''???''
        end 
       end 
      end 
     as vid_doc,
     case when (p.id_pref=10 or p.id_pref=1099) then ''AE''::varchar else 
       case when (p.id_pref=20 or p.id_pref=2099) then ''PE''::varchar else 
        case when (p.id_pref=901) then ''оЕМЪ''::varchar else 
         case when (p.id_pref=902) then ''хМТК''::varchar else 
                              ...
                              from
                               ( select id_client,id_pref,mmgg,
                                ...
                                ( select id_client,id_pref,mmgg,
                                  ...
                                  sum(suma_zpdv) as suma_zpdv
                                  from seb_opl_tmp o
                                  where mmgg=pmmgg  and vid_doc=''аЮМЙ'' group by id_client,id_pref,mmgg
                                ) as bank
...
update seb_obr_all_tmp set  
   ...
   deb_pm17v=0,  deb_pm17e=0, deb_pm17pdv=0,  deb_km17v=0, deb_km17e=0, deb_km17pdv=0,
   deb_pm18v=0,  deb_pm18e=0, deb_pm18pdv=0,  deb_km18v=0, deb_km18e=0, deb_km18pdv=0;
...
update seb_obr_all_tmp set deb_pm16v=-(h.b_dtval+h.b_dtval_tax),
   ...
   where h.id_client=seb_obr_all_tmp.id_client and h.id_pref=seb_obr_all_tmp.id_pref 
        and h.hmmgg=''2016-01-01'';
                             -- 2016
update seb_obr_all_tmp set deb_pm17v=-(h.b_dtval+h.b_dtval_tax),
                   deb_pm17e=-h.b_dtval, 
                   deb_pm17pdv=-h.b_dtval_tax,
                   deb_km17v=-(h.e_dtval+h.e_dtval_tax),
                   deb_km17e=-h.e_dtval, 
                   deb_km17pdv=-h.e_dtval_tax,
                    b_kred= case when ( h.b_ktval<>0) then (h.b_ktval-coalesce(h.b_dtval,0)) else 
                             case when ( h.b_ktval=0 and h.b_dtval<0) then (h.b_ktval-coalesce(h.b_dtval,0)) else 0  end 
                           end ,
                   b_kred_tax=case when ( h.b_ktval_tax<>0) then (h.b_ktval_tax-coalesce(h.b_dtval_tax,0)) else 
   ...
   where h.id_client=seb_obr_all_tmp.id_client and h.id_pref=seb_obr_all_tmp.id_pref 
        and h.hmmgg=''2017-01-01'';


update seb_obr_all_tmp set deb_pm18v=-(h.b_dtval+h.b_dtval_tax),
                   deb_pm18e=-h.b_dtval, 
                   deb_pm18pdv=-h.b_dtval_tax,
                   deb_km18v=-(h.e_dtval+h.e_dtval_tax),
                   deb_km18e=-h.e_dtval, 
                   deb_km18pdv=-h.e_dtval_tax,
                    b_kred= case when ( h.b_ktval<>0) then (h.b_ktval-coalesce(h.b_dtval,0)) else 
                             case when ( h.b_ktval=0 and h.b_dtval<0) then (h.b_ktval-coalesce(h.b_dtval,0)) else 0  end 
                           end ,
                   b_kred_tax=case when ( h.b_ktval_tax<>0) then (h.b_ktval_tax-coalesce(h.b_dtval_tax,0)) else 
...
if date_part(''year'',pmmgg) = 2017 then
update seb_obr_all_tmp set kr_zpmv=0,deb_zpmv=deb_zpmv+kr_zpmv,deb_pm17v=coalesce(deb_pm17v,0)+kr_zpmv,
                       kr_zpme=0,deb_zpme=deb_zpme+kr_zpme, deb_pm17e=coalesce(deb_pm17e,0)+kr_zpme,
                       kr_zpmpdv=0,deb_zpmpdv=deb_zpmpdv+kr_zpmpdv,deb_pm17pdv=coalesce(deb_pm17pdv,0)+kr_zpmpdv
             where kr_zpmv<0;


update seb_obr_all_tmp set kr_zkmv=0, deb_kmv=deb_kmv+kr_zkmv,deb_km17v=coalesce(deb_km17v,0)+kr_zkmv,
                           kr_zkme=0,deb_kme=deb_kme+kr_zkme, deb_km17e=coalesce(deb_km17e,0)+kr_zkme,
                           kr_zkmpdv=0,deb_kmpdv=deb_kmpdv+kr_zkmpdv,deb_km17pdv=coalesce(deb_km17pdv,0)+kr_zkmpdv
             where kr_zkmv<0;
...
update seb_obr_all_tmp set kr_zkmv=0, deb_kmv=deb_kmv+kr_zkmv,deb_km17v=coalesce(deb_km17v,0)+coalesce(kr_zkmv,0),
                           kr_zkme=0,deb_kme=deb_kme+kr_zkme, deb_km17e=coalesce(deb_km17e,0)+coalesce(kr_zkme,0),
                           kr_zkmpdv=0,deb_kmpdv=deb_kmpdv+kr_zkmpdv,deb_km17pdv=coalesce(deb_km17pdv,0)+coalesce(kr_zkmpdv,0)
             where kr_zkmv<0;


end if;
if date_part(''year'',pmmgg) = 2018 then
update seb_obr_all_tmp set kr_zpmv=0,deb_zpmv=deb_zpmv+kr_zpmv,deb_pm18v=coalesce(deb_pm18v,0)+kr_zpmv,
                       kr_zpme=0,deb_zpme=deb_zpme+kr_zpme, deb_pm18e=coalesce(deb_pm18e,0)+kr_zpme,
                       kr_zpmpdv=0,deb_zpmpdv=deb_zpmpdv+kr_zpmpdv,deb_pm18pdv=coalesce(deb_pm18pdv,0)+kr_zpmpdv
             where kr_zpmv<0;


update seb_obr_all_tmp set kr_zkmv=0, deb_kmv=deb_kmv+kr_zkmv,deb_km18v=coalesce(deb_km18v,0)+kr_zkmv,
                           kr_zkme=0,deb_kme=deb_kme+kr_zkme, deb_km18e=coalesce(deb_km18e,0)+kr_zkme,
                           kr_zkmpdv=0,deb_kmpdv=deb_kmpdv+kr_zkmpdv,deb_km18pdv=coalesce(deb_km18pdv,0)+kr_zkmpdv
             where kr_zkmv<0;
...
update seb_obr_all_tmp set kr_zkmv=0, deb_kmv=deb_kmv+kr_zkmv,deb_km18v=coalesce(deb_km18v,0)+coalesce(kr_zkmv,0),
                           kr_zkme=0,deb_kme=deb_kme+kr_zkme, deb_km18e=coalesce(deb_km18e,0)+coalesce(kr_zkme,0),
                           kr_zkmpdv=0,deb_kmpdv=deb_kmpdv+kr_zkmpdv,deb_km18pdv=coalesce(deb_km18pdv,0)+coalesce(kr_zkmpdv,0)
             where kr_zkmv<0;
...
if pmmgg=''2017-01-01'' then
 raise notice ''update all 2017-01-01'';
 update seb_obr_all_tmp set deb_pm16v=deb_pm16v+deb_pm17v,
                   deb_pm16e=deb_pm16e+deb_pm17e, 
                   deb_pm16pdv=deb_pm16pdv+deb_pm17pdv;

 update seb_obr_all_tmp set deb_pm17v=0,
                   deb_pm17e=0, deb_pm17pdv=0;
end if;


if pmmgg=''2018-01-01'' then
 raise notice ''update all 2018-01-01'';
 update seb_obr_all_tmp set deb_pm17v=deb_pm17v+deb_pm18v,
                   deb_pm17e=deb_pm17e+deb_pm18e, 
                   deb_pm17pdv=deb_pm17pdv+deb_pm18pdv;

 update seb_obr_all_tmp set deb_pm18v=0,
                   deb_pm18e=0, deb_pm18pdv=0;
end if;
...
, deb_pm17v ,  deb_pm17e ,  deb_pm17pdv ,  deb_km17v ,  deb_km17e ,  deb_km17pdv
, deb_pm18v ,  deb_pm18e ,  deb_pm18pdv ,  deb_km18v ,  deb_km18e ,  deb_km18pdv
)
 to ''||quote_literal(tabl)||'' with delimiter as ''||quote_literal(del)||'' null as ''|| quote_literal(nul);
 select into flag_export value_ident from syi_sysvars_tbl where ident=''flag_exp'';
  if flag_export is null or flag_export=''1'' then
...
-----------------------------------------
 ...
 from (select b.id_client, mm.id_type_eqp as id_type_meter ,max(mp.id_point) as id_point,trim(regexp_replace(trim(eq.num_eqp),''(М|Д)$'',''''))::varchar as num_eqp,
       ...
       left join eqm_meter_h as mm on (mm.code_eqp = mp.id_meter and mm.dt_b <= bs.dt_begin and ( mm.dt_e is null or mm.dt_e > bs.dt_begin ) ) 
        ...
        left join (select b.id_client, trim(regexp_replace(trim(eq.num_eqp),''(М|Д)$'',''''))::varchar as num_eqp,
...
insert into cmi_place_tbl (id,name) values (1,'на опор╕');
insert into cmi_place_tbl (id,name) values (2,'вв╕дний автомат');
insert into cmi_place_tbl (id,name) values (3,'на ╖золяторах ввода ');
insert into cmi_place_tbl (id,name) values (4,'в щитов╕й');
insert into cmi_place_tbl (id,name) values (5,'в КТП');
insert into cmi_place_tbl (id,name) values (6,'на обладнанн╕ споживача');
insert into cmi_place_tbl (id,name) values (7,'╕нше');
...
insert into cmi_reason_tbl (id,name) values (10,'закритий догов╕р');
...
insert into cmi_transmiss_tbl (id,name) values (1,'особисто (п╕д п╕дпис)');
...
insert into cmi_transmiss_tbl (id,name) values (11,'в╕д п╕дпису в╕дмовився');
