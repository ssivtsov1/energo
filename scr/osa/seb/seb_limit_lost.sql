...
select kodres,mmgg1, coalesce(lost_nolost,0),coalesce(id_voltage,0), cl.code, cl.id,   
   0 as id_fider ,Null as fider ,  id_point, name_eqp, num_eqp,  
    sum(dem) as dem , sum(kvt) as kvt,id_voltage as set_power,connect_power  
  ...
  group by id_point, name_eqp, num_eqp,id_voltage,lost_nolost,cl.id, cl.code ,id_voltage,connect_power
...
update  seb_abon_lost_tmp set kind_dep=c.kind_dep from clm_client_tbl c where c.id= seb_abon_lost_tmp.id_client;
  
raise notice ''losts'';
 tabl=path_export||kodres||''lst_lost.TXT'';
 del=''@''; nul='''';
 SQL=''copy  seb_abon_lost_tmp ( kind_dep,  mmgg , losts,  voltage  ,   code ,
     fider,  id_point,  point_name, num_eqp,  
    demand     , sum_demand,set_power,connect_power) 
...
mmgg2=eom(mmgg1);
--ret=bal_abon_find_fun();
delete from  seb_abon_limit_tmp; 
select value_ident into kodres from syi_sysvars_tbl where ident=''kod_res''; 
...
select cl.kind_dep,h.mmgg, h.reg_num,h.reg_date,h.idk_document,
 cl.code, h.dt,h.id_area ,null, h.month_limit, h.value_dem 
  from clm_client_tbl as cl,
  (select h.mmgg,h.id_client, h.reg_num,h.reg_date,h.idk_document,
    h.dt,d.id_area ,null, d.month_limit, d.value_dem 
    from acm_headdemandlimit_tbl h,acd_demandlimit_tbl d 
     where d.id_doc=h.id_doc and h.idk_document=600 ) as h
...
raise notice ''losts'';
 tabl=path_export||kodres||''limit.TXT'';
 del=''@''; nul='''';
 SQL=''copy  seb_abon_limit_tmp ( 
 kind_dep,  mmgg ,  reg_num ,  reg_date , idk_document, 
     code ,  dt,id_area, id_point, month_limit,value_dem
) 
  to ''||quote_literal(tabl)||'' with delimiter as ''||quote_literal(del)||'' null as ''|| quote_literal(nul);
