...
--    group by cp.id_section
   loop
     ...
     Raise Notice ''Налоговая накладная для населения .'';
...
--      vsum_tax = (vsaldo.nar_pdv+vsaldo.ekt_pdv-vsaldo.bkt_pdv);
   ...
   */
     ...
     if found then 
       Raise Notice ''Налоговая накладная для населения в этом месяце уже сформирована.'';
     else
      ...
      if vsum_all > 0 then 
        ...
        insert into acd_tax_tbl(id_doc,dt_bill,unit,unit_kod,uktzed,text,demand_val,sum_val7,tariff)
        values(vid_new,preg_date,''кВт·год'',''0415'',''2716000000'',''За активну електроенергiю '',vdemand,vsum_all,vtariff);
