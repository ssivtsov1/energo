...
create or replace function bal_katacc_fun(int,date,date) Returns boolean As '
Declare
...
-- процедура Кати из onetree.sql, изначально - выборка данных для отчета о потреблении
-- возьмем только точки учета, счетчики и измерительные тр. 
...
delete from act_met_kndzn_tbl;
 ...
 select into idcl getsysvar(''id_bal'');
...
(select id,code_eqp,case when dt_b<=dtb then dtb else dt_b end as dt_b
  ...
  where id_client=idcl and coalesce(dt_e,dte)>dtb and dt_b<dte and ((id =ptree) or (ptree is null))) as a1
 inner join 
