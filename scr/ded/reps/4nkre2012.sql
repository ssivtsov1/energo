...
--drop table rep_4nkre2012_tbl;

create table rep_4nkre2012_tbl (
id serial,
...
CREATE or replace FUNCTION rep_4nkre2012_fun(date,int)
RETURNS int
AS                                                                                              
  '
  declare
...
--   pr=rep_nkre4(pmmgg);
  ...
  delete from rep_4nkre2012_tbl where mmgg = pmmgg;
 
  insert into rep_4nkre2012_tbl (mmgg) select pmmgg;


raise notice ''update 1 kl '';

   update rep_4nkre2012_tbl set 
   ...
   sum300=s300,dem300=d300, sum302=s302,dem302=d302, sum306=s306,dem306=d306,sum308=s308,dem308=d308,
  ...
  (select 
         ...
         coalesce(sum(CASE WHEN ident =''tgr5'' THEN demand_tcl1 ELSE 0 END )/1000,0) as d44,
	 ...
	 coalesce(sum(CASE WHEN ident =''tgr1'' THEN sum_tcl1 ELSE 0 END )/1000,0) as s04,
         ...
         coalesce(sum(CASE WHEN ident =''tgr5'' THEN sum_tcl1 ELSE 0 END )/1000,0) as s44,


         coalesce(sum(CASE WHEN ident = ''tgr8_12'' THEN demand_tcl1 ELSE 0 END )/1000,0) as d302,
         coalesce(sum(CASE WHEN ident = ''tgr8_12'' THEN sum_tcl1 ELSE 0 END )/1000,0) as s302,

         coalesce(sum(CASE WHEN ident = ''tgr8_22'' THEN demand_tcl1 ELSE 0 END )/1000,0) as d308,
         coalesce(sum(CASE WHEN ident = ''tgr8_22'' THEN sum_tcl1 ELSE 0 END )/1000,0) as s308,
...
--  left join cla_param_tbl as p on (p.id=ph.id_extra)
 ...
 where rep_4nkre2012_tbl.mmgg = pmmgg;
...
raise notice ''update 2 kl '';
   ...
   sum358=s358,dem358=d358, sum362=s362,dem362=d362, sum364=s364,dem364=d364,
   sum366=s366,dem366=d366, sum368=s368,dem368=d368, sum372=s372,dem372=d372,
   sum374=s374,dem374=d374, sum376=s376,dem376=d376, sum378=s378,dem378=d378,
   sum384=s384,dem384=d384, sum386=s386,dem386=d386, sum388=s388,dem388=d388,
   sum392=s392,dem392=d392, sum394=s394,dem394=d394, sum396=s396,dem396=d396,
   sum402=s402,dem402=d402, sum404=s404,dem404=d404, sum406=s406,dem406=d406,
  ...
  (select 
         ...
         coalesce(sum(CASE WHEN ident =''tgr5'' THEN demand_tcl1 ELSE 0 END )/1000,0) as d84,
	 ...
	 coalesce(sum(CASE WHEN ident =''tgr1'' THEN sum_tcl1 ELSE 0 END )/1000,0) as s54,
         ...
         coalesce(sum(CASE WHEN ident =''tgr5'' THEN sum_tcl1 ELSE 0 END )/1000,0) as s84,
...
raise notice ''update naselen nopoint'';
  ...
  update rep_4nkre2012_tbl set 
   ...
   sum358=sum358+s358,dem358=dem358+d358, sum362=sum362+s362,dem362=dem362+d362, sum364=sum364+s364,dem364=dem364+d364,
   sum366=sum366+s366,dem366=dem366+d366, sum368=sum368+s368,dem368=dem368+d368, sum372=sum372+s372,dem372=dem372+d372,
   sum374=sum374+s374,dem374=dem374+d374, sum376=sum376+s376,dem376=dem376+d376, sum378=sum378+s378,dem378=dem378+d378,
   sum384=sum384+s384,dem384=dem384+d384, sum386=sum386+s386,dem386=dem386+d386, sum388=sum388+s388,dem388=dem388+d388,
   sum392=sum392+s392,dem392=dem392+d392, sum394=sum394+s394,dem394=dem394+d394, sum396=sum396+s396,dem396=dem396+d396,
   sum402=sum402+s402,dem402=dem402+d402, sum404=sum404+s404,dem404=dem404+d404, sum406=sum406+s406,dem406=dem406+d406,
   sum410=sum410+s410,dem410=dem410+d410, sum412=sum412+s412,dem412=dem412+d412, sum414=sum414+s414,dem414=dem414+d414,

   sum340=sum340+s352+s354+s356+s358+s370+s398,
   sum342=sum342+s362+s364+s366+s368,

   dem340=dem340+d352+d354+d356+d358+d370+d398,
   dem342=dem342+d362+d364+d366+d368
...
raise notice ''hoznuj'';
  ...
  --if (select coalesce(flag_hand,0) from sebi_nkre4 where code = 578)=0 then 
    ...
    update rep_4nkre2012_tbl set sum604=s604,dem604=d604, sum606=s606,dem606=d606
...
/*
    update rep_4nkre2012_tbl set sum656=s656,dem656=d656, sum658=s658,dem658=d658,
...
--  if (select coalesce(flag_hand,0) from sebi_nkre4 where code = 700)=0 then 
  ...
  update rep_4nkre2012_tbl set sum632=s632,dem632=d632, sum634=s634,dem634=d634
   from
  (select 
...
--  if (select coalesce(flag_hand,0) from sebi_nkre4 where code = 730)=0 then 
  ...
  update rep_4nkre2012_tbl set sum638=s638,dem638=d638, sum640=s640,dem640=d640,
...
--  if (select coalesce(flag_hand,0) from sebi_nkre4 where code = 784)=0 then 
  ...
  update rep_4nkre2012_tbl set sum662=s662,dem662=d662, sum664=s664,dem664=d664,sum666=s666,dem666=d666
   from
  (select 
...
/*
 ...
 -- ТВЕ/Тяга
  ...
  update rep_4nkre2012_tbl set  sum564=s564,dem564=d564
  from
  (select 
...
--  if (select coalesce(flag_hand,0) from sebi_nkre4 where code = 554)=0 then 
  ...
  update rep_4nkre2012_tbl set  sum574=s574,dem574=d574
  from
  (select 
  ...
  update rep_4nkre2012_tbl set  sum582=s582,dem582=d582
  from
  (select 
...
-- update rep_4nkre2012_tbl set sum670=summa,dem670=demand from sebd_nkre4 where code_nkre4=670 and sebd_nkre4.mmgg=pmmgg;
--end if;
 -----------------------------------------------------------------------------------------

 if (pfile = 1) then

  CREATE temp TABLE rep_4nkre2012_tmp AS select * from rep_4nkre2012_tbl where mmgg = pmmgg;
