--drop function oabn_tree(int,date,date);
create or replace function oabn_tree(int,date,date) Returns boolean As '
Declare
idcl Alias for $1;
dtb Alias for $2;
dte Alias for $3;
...
select distinct a_12.id,a_12.code_eqp,a_12.code_eqp_e,a3.type_eqp
  ,case when a3.dt_b<=a_12.dt_b then a_12.dt_b else a3.dt_b end as dt_b
  ,case when a3.dt_e>=a_12.dt_e then a_12.dt_e else a3.dt_e end as dt_e
...
(select distinct a1.id,a2.code_eqp,a2.code_eqp_e
  ...
  ,a1.id_client,a2.line_no
     from 
(select h1.id,h1.code_eqp,case when h1.dt_b<=dtb then dtb else h1.dt_b end as dt_b
  ,case when coalesce(h1.dt_e,dte)>=dte then dte else h1.dt_e end as dt_e
  ,h1.id_client from eqm_tree_h as h1
  where h1.id_client=idcl and coalesce(h1.dt_e,dte)>dtb and h1.dt_b<dte) as a1
 inner join 
(select h1.id_tree,h1.code_eqp,case when h1.dt_b<=dtb then dtb else h1.dt_b end as dt_b
  ,case when coalesce(h1.dt_e,dte)>=dte then dte else h1.dt_e end as dt_e
  ,h1.code_eqp_e,h1.line_no from eqm_eqp_tree_h as h1 
  where coalesce(h1.dt_e,dte)>dtb and h1.dt_b<dte) as a2 

 on (a1.id=a2.id_tree and (a1.dt_b<a2.dt_e and a1.dt_e>a2.dt_b))
 ) as a_12
 inner join 
(select h1.id,h1.type_eqp,h1.loss_power
  ,case when h1.dt_b<=dtb then dtb else h1.dt_b end as dt_b
  ,case when coalesce(h1.dt_e,dte)>=dte then dte else h1.dt_e end as dt_e ,
 h1.id_department 

 from eqm_equipment_h as h1 where 
 coalesce(h1.dt_e,dte)>dtb and h1.dt_b<dte) as a3 

  on (a3.id=a_12.code_eqp and a_12.dt_b<a3.dt_e and a_12.dt_e>a3.dt_b) 

 left join (select h1.code_eqp,h1.id_client as id_rclient,case when h1.dt_b<=dtb 
  then dtb else h1.dt_b end as dt_b
 ,case when coalesce(h1.dt_e,dte)>=dte then dte else h1.dt_e end as dt_e
  from eqm_eqp_use_h as h1 where coalesce(h1.dt_e,dte)>dtb and h1.dt_b<dte) as a4
 on (a3.id=a4.code_eqp and a3.dt_b<a4.dt_e and a3.dt_e>a4.dt_b)
...
(select id_tree,code_eqp,case when dt_b<=dtb then dtb else dt_b end as dt_b
 ...
 inner join 
(select id,type_eqp,loss_power
  ,case when h.dt_b<=dtb then dtb else h.dt_b end as dt_b
  ,case when coalesce(h.dt_e,dte)>=dte then dte else h.dt_e end as dt_e 
 from eqm_equipment_h  h
where 
 coalesce(h.dt_e,dte)>dtb and h.dt_b<dte) as a3 
  on (a3.id=a_12.code_eqp and a_12.dt_b<a3.dt_e and a_12.dt_e>a3.dt_b)  
 inner join
...
------- sel_point_tree
  insert into act_point_branch_tbl(id_point,id_p_point,dat_b,dat_e,id_client
     ,id_rclient,id_tree) 
  select a.code_eqp,null as id_p_point,a.dat_b,a.dat_e,a.id_client
   ,a.id_rclient,a.id_tree from act_eqp_branch_tbl as a where a.type_eqp=12;
--act_meter_pnt_tbl
