...
update act_difmetzone_tbl set 
  demand_calc=round(d.fact_demand*percent/100,0),
 fact_demand_p=demand_p-round(d.fact_demand*percent/100,0)
-- demand_do=demand_p
  from 
  ( select id_point,kind_energy,id_zone,min(dat_b,test.dt_b) as dat_b,max(dat_e,test.dt_e) as dat_e,sum(fact_demand)  as fact_demand
...
(select distinct c1.id_point,c1.id_meter,c2.kind_energy
 ...
 insert into act_pwr_demand_tbl(id_point,kind_energy,dat_b,dat_e
     ,fact_demand,sum_demand,ident)
select b.id_point,b.kind_energy,b.dat_b,b.dat_e
     ,sum(round(c.ldemandg::numeric/c.pdays*date_mi(b.dat_e,b.dat_b),0))
     ,sum(round(c.ldemandg::numeric/c.pdays*date_mi(b.dat_e,b.dat_b),0)),3 
   from 
(select a.id_point,a.kind_energy  
   ,case when (a.dat_b<coalesce(b.dat_b,a.dat_b) and a.dat_e=coalesce(b.dat_e,a.dat_e))  
    then a.dat_b 
...
(select distinct c1.id_point,c1.id_meter,c2.kind_energy
  ...
  from act_meter_pnt_tbl as c1 inner join act_met_knd as c2 
   ...
   and (a.dat_b<b.dat_b or a.dat_e>b.dat_e)) ) as b 
     inner join 
        (select * from act_pnt_cntavgg where ldemandg<>0 and pdays<>0) as c 
     on (b.id_point=c.id_point and b.dat_b<c.dat_e and b.dat_e>c.dat_b) 
     group by b.id_point,b.kind_energy,b.dat_b,b.dat_e;
 ...
 insert into act_pwr_demand_tbl(id_point,kind_energy,dat_b,dat_e
     ,fact_demand,sum_demand,ident)
select b.id_point,b.kind_energy,b.dat_b,b.dat_e
     ,sum(round(c.ldemandg/c.pdays*date_mi(c.dat_e,c.dat_b),0))
     ,sum(round(c.ldemandg/c.pdays*date_mi(c.dat_e,c.dat_b),0)),3 
     from 
 (select a.id_point,a.kind_energy,a.dat_b,a.dat_e from 
 (select a1.id_point,a2.kind_energy,a1.dat_b,a1.dat_e from 
  act_point_branch_tbl as a1 inner join act_pnt_knd as a2 
...
update act_comp_percent set wp_parent =fact_par.fact,add_wp=(perc/100)*fact_par.fact from  
 (select id_point,sum(fact_demand) as fact from act_pwr_demand_tbl where kind_energy=1 
  group by id_point) fact_par
 where fact_par.id_point=act_comp_percent.id_p_point;

update act_losts_eqm_tbl set own_wp=c.add_wp 
from  act_comp_percent c,act_eqp_branch_tbl b1,
act_eqp_branch_tbl b2
where c.id_point=act_losts_eqm_tbl.id_point
and b1.code_eqp=c.id_p_point
and b2.code_eqp=c.id_eqp
and b1.id_client=b2.id_client
...
update act_losts_eqm_tbl set own_wp=0.00 where own_wp is null;
 ...
 for test in select * from act_comp_percent order by id_point,dat_b,dat_e loop
      raise notice ''AAAfter calc percent  test.id_point  % ,id_p_point  %, id_eqp %, 
