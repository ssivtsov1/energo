...
//  Param->Name="lnow";
  ...
  if ((ZQXLRepMain->FieldByName("sum_infl_a")->AsFloat!=0)&&
   ...
   if ((ZQXLRepMain->FieldByName("sum_3_proc_a")->AsFloat!=0)&&             //from yana
     ...
     {
      xlReport->ParamByName["pgi_count"]->Value = "(2 арк.)";
      xlReport->ParamByName["3_proc_add"]->Value = "(Додаток 3.1, 3.2)";
      xlReport->ParamByName["add31"]->Value = "Додаток 3.1";
      xlReport->ParamByName["add32"]->Value = "Додаток 3.2";

     }
  else
     {
      xlReport->ParamByName["pgi_count"]->Value = "(1 арк.)";
      xlReport->ParamByName["3_proc_add"]->Value = "(Додаток 3)";

      if (ZQXLRepMain->FieldByName("sum_3_proc_a")->AsFloat!=0)
        {
          xlReport->ParamByName["add31"]->Value = "Додаток 3";
          xlReport->ParamByName["add32"]->Value = " - - - - - ";
        }

      if (ZQXLRepMain->FieldByName("sum_3_proc_r")->AsFloat!=0)
        {
          xlReport->ParamByName["add32"]->Value = "Додаток 3";
