...
ZQBillItog->Database=TWTTable::Database;
...
//   ZQBillSumm->MacroByName("ttt")->AsString="bs.demand_val::numeric";
   ...
   ZQBillSumm2kr_point_2013->ParamByName("doc")->AsInteger=id_bill;
     ZQBillItog->ParamByName("doc")->AsInteger=id_bill;
   ...
   {
   // ShowMessage( ZQBillItog->Sql->Text);
...
//     lBillDate->Caption=ZQBill->FieldByName("reg_date")->AsString;
     ...
     LOpl->Caption=FormatFloat("0.00",ZQBillItog->FieldByName("spay")->AsFloat-ZQBillItog->FieldByName("sbill")->AsFloat);
     if (ZQBillItog->FieldByName("billsumm")->AsFloat \
            -ZQBillItog->FieldByName("spay")->AsFloat+ \
            ZQBillItog->FieldByName("sbill")->AsFloat>0)
      {
      if (ZQBillItog->FieldByName("billsumm")->AsFloat \
            -ZQBillItog->FieldByName("spay")->AsFloat+ \
            ZQBillItog->FieldByName("sbill")->AsFloat>ZQBillItog->FieldByName("billsumm")->AsFloat)
               LItog->Caption=FormatFloat("0.00",ZQBillItog->FieldByName("billsumm")->AsFloat);
        else
             LItog->Caption=FormatFloat("0.00",ZQBillItog->FieldByName("billsumm")->AsFloat-ZQBillItog->FieldByName("spay")->AsFloat+ZQBillItog->FieldByName("sbill")->AsFloat);
      }
     else
      {  if (ZQBillItog->FieldByName("billsumm")->AsFloat \
            -ZQBillItog->FieldByName("spay")->AsFloat+ \
            ZQBillItog->FieldByName("sbill")->AsFloat<=0)
            LItog->Caption=FormatFloat("0.00",0);
      }
...
//     lChnoeName->Caption=ZQBill->FieldByName("chnoename")->AsString;
     ...
     if (ZQBill->FieldByName("last_demand")->AsFloat > 0)
     {
      ...
      qrlWarning1->Caption ="У разі несплати рахунка протягом "+ZQBill->FieldByName("day_pay_bill")->AsString +" банківських днів будуть вжиті заходи щодо припинення електропостачання вашого підприємства відповідно з п. 7.5 (3) правил користування електричною енергією.";
   ...
   if ((kind==20)||(kind==201)) {
        QRLKindEnergy->Caption= "За перетікання реактивної електроенергії";
...
if (PrintBill_p1->DataSet->FieldByName("kind_energy")->AsInteger==2)
 QRLEnergy->Caption="Перетікання реактивної енергії ";
