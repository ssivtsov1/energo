...
//---------------------------------------------------------------------------
void __fastcall TClientPar::TreeChange(TObject *Sender,
      TTreeNode *Node)
{
...
//---------------------------------------------------------------------------
void __fastcall TClientPar::AddButClick(TObject *Sender)
{
Memo1->Clear();
Edit1->Text="";
Edit4->Text="0";
...
//---------------------------------------------------------------------------
void __fastcall TClientPar::InsButClick(TObject *Sender)
{
flag=0;
NextPar->Items->Clear();
Memo2->Clear();
...
//---------------------------------------------------------------------------
void __fastcall TClientPar::FormCreate(TObject *Sender)
{
...
//---------------------------------------------------------------------------
int __fastcall TClientPar::GetIndex(AnsiString str, TTreeView *Tree)
{
int i=-1,f=0;
...
void __fastcall TClientPar::MakeTree()
{
Tree->Items->Clear();
  Client->Query->Sql->Clear();
...
Client->Query->Next();
    ...
    Client->Query->Sql->Clear();
    Client->Query->Sql->Add("select id,name from cla_param_tbl order by id");
    Client->Query->ExecSql();
    Client->Query->Active=true;
//Именование элементов дерева
//where id="+Tree->Items->Item[i]->Text)
for (int i=0;i<Tree->Items->Count;i++)
   {
    TLocateOptions SearchOptions;
   SearchOptions.Clear();
   Client->Query->Locate("id",Tree->Items->Item[i]->Text,  SearchOptions);
    //Client->Query->FindField("id")->AsInteger=StrToInt(Tree->Items->Item[i]->Text);
    Tree->Items->Item[i]->Text=Client->Query->FieldByName("name")->AsString;
   }
Client->Query->Active=false;
//Tree->FullExpand();
}
//-----------------------------------------------------------------------------------------------------

void __fastcall TClientPar::FormActivate(TObject *Sender)
{
ComboBox1->Clear();
ComboBox2->Clear();
ComboBox3->Clear();
  ...
  Client->Query->Sql->Clear();
  Client->Query->Sql->Add("select * from syi_type_tbl where name is not null");
  Client->Query->ExecSql();
  Client->Query->Active=true;
...
void __fastcall TClientPar::SpeedButton5Click(TObject *Sender)
{
Tree->SetFocus();
}
//---------------------------------------------------------------------------

void __fastcall TClientPar::SpeedButton4Click(TObject *Sender)
{
...
void __fastcall TClientPar::DelButClick(TObject *Sender)
{
if (id_param!=0)
  {
  Client->Query->Sql->Clear();
...
void __fastcall TClientPar::RadioButton1Click(TObject *Sender)
{
 flag=1;
 id_next=0;
 NextPar->Enabled=false;
...
void __fastcall TClientPar::RadioButton2Click(TObject *Sender)
{
 flag=0;
 NextPar->Enabled=true;
}
...
void __fastcall TClientPar::NextParChange(TObject *Sender,
...
void __fastcall TClientPar::SpeedButton7Click(TObject *Sender)
{
int id_type=0;
...
void __fastcall TClientPar::SpeedButton9Click(TObject *Sender)
{
  int id=id_param;
...
void __fastcall TClientPar::SpeedButton8Click(TObject *Sender)
{
  int id=id_param;
...
void __fastcall TClientPar::FormClose(TObject *Sender,
      TCloseAction &Action)
{
...
void __fastcall TClientPar::N2Click(TObject *Sender)
{
ClientPar->Close();
}
//---------------------------------------------------------------------------
void __fastcall TClientPar::Edit3Change(TObject *Sender)
{
AnsiString ttt="";
...
void __fastcall TClientPar::Memo2Change(TObject *Sender)
{
...
//---------------------------------------------------------------------------
int __fastcall TClientPar::Count(AnsiString tmp)
{
int cnt=0;
...
void __fastcall TClientPar::Edit4Change(TObject *Sender)
{
AnsiString ttt="";
...
void __fastcall TClientPar::Memo1Change(TObject *Sender)
{
...
void __fastcall TClientPar::Memo3Change(TObject *Sender)
{
...
void __fastcall TClientPar::SpeedButton1Click(TObject *Sender)
{
ClientPar->Close();
...
int rez=0;
if (CheckBox1->Checked) rez=Client->CreateParamN(id_param);
else rez=Client->CreateParam(id_param,ttt);

 if (rez==2) Client->DrawParam(id_param,3);
