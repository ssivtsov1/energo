...
// Определим, есть ли вводы на ПС
  ...
  if (selected==0)
  {
   ...
   ( select code_eqp, coalesce(id_point,101202) as id_point, name, coalesce(sum(demand),0)::::int as demand, coalesce(sum(losts),0)::::int as losts \
     from  bal_grp_tree_conn_tmp  where type_eqp=15 and id_voltage not in (4,42) \
     and mmgg = :dat group by code_eqp,id_point,name order by id_point) as gt \
...
void __fastcall TfBalansRep::FizAbonInfo(TDateTime mmgg)
{
  ...
  (select count(*) as cnt_load from acm_privdem_tbl where  mmgg = :mmgg and coalesce(value,0)<>0 ) as s3, \
  ...
  (select count(*) as cnt_load_connect from acm_privdem_tbl where mmgg = :mmgg and id_eqpborder is not null and coalesce(value,0)<>0 ) as s7, \
...
// Определим, есть ли вводы на ПС
  ...
  if (selected==0)
  {
   ...
   ( select mmgg, code_eqp, id_point, name, coalesce(sum(demand),0)::::int as demand, coalesce(sum(losts),0)::::int as losts \
     from bal_grp_tree_conn_tbl where type_eqp=15 and id_voltage not in (4,42) and date_part('year',mmgg::::date) = date_part('year',:dat::::date)  and date_part('month',mmgg::::date) = 1 \
     and id_bal = :res \
  ...
  and gt.id_bal = :res  and gt2.id_bal = :res \
   ...
   from \
   ( select mmgg, code_eqp, id_point, name, coalesce(sum(demand),0)::::int as demand, coalesce(sum(losts),0)::::int as losts \
     from bal_grp_tree_conn_tbl where type_eqp=15 and id_voltage not in (4,42) and date_part('year',mmgg::::date) = date_part('year',:dat::::date)  and date_part('month',mmgg::::date) = 2 \
     and id_bal = :res \
     group by code_eqp,id_point,name, mmgg order by id_point ) as gt \
    left join bal_demand_tbl as bd  on (bd.id_point=gt.id_point and bd.mmgg = gt.mmgg and bd.id_bal = :res) \
   union \
  ...
  and gt.id_bal = :res  and gt2.id_bal = :res \
   ...
   from \
   ( select mmgg, code_eqp, id_point, name, coalesce(sum(demand),0)::::int as demand, coalesce(sum(losts),0)::::int as losts \
     from bal_grp_tree_conn_tbl where type_eqp=15 and id_voltage not in (4,42) and date_part('year',mmgg::::date) = date_part('year',:dat::::date)  and date_part('month',mmgg::::date) = 3 \
     and id_bal = :res \
     group by code_eqp,id_point,name, mmgg order by id_point) as gt \
    left join bal_demand_tbl as bd  on (bd.id_point=gt.id_point and bd.mmgg = gt.mmgg and bd.id_bal = :res) \
   union \
  ...
  and gt.id_bal = :res  and gt2.id_bal = :res \
   ...
   from \
   ( select mmgg, code_eqp, id_point, name, coalesce(sum(demand),0)::::int as demand, coalesce(sum(losts),0)::::int as losts \
     from bal_grp_tree_conn_tbl where type_eqp=15 and id_voltage not in (4,42) and date_part('year',mmgg::::date) = date_part('year',:dat::::date)  and date_part('month',mmgg::::date) = 4 \
     and id_bal = :res \
     group by code_eqp,id_point,name, mmgg order by id_point ) as gt \
    left join bal_demand_tbl as bd  on (bd.id_point=gt.id_point and bd.mmgg = gt.mmgg and bd.id_bal = :res) \
   union \
  ...
  and gt.type_eqp=12 and date_part('year',gt.mmgg::::date) = date_part('year',:dat::::date)  and date_part('month',gt.mmgg::::date) = 4  \
   ...
   from \
   ( select mmgg, code_eqp, id_point, name, coalesce(sum(demand),0)::::int as demand, coalesce(sum(losts),0)::::int as losts \
     from bal_grp_tree_conn_tbl where type_eqp=15 and id_voltage not in (4,42) and date_part('year',mmgg::::date) = date_part('year',:dat::::date)  and date_part('month',mmgg::::date) = 5 \
     and id_bal = :res \
     group by code_eqp,id_point,name, mmgg order by id_point ) as gt \
    left join bal_demand_tbl as bd  on (bd.id_point=gt.id_point and bd.mmgg = gt.mmgg and bd.id_bal = :res) \
   union \
  ...
  and gt.type_eqp=12 and date_part('year',gt.mmgg::::date) = date_part('year',:dat::::date)  and date_part('month',gt.mmgg::::date) = 5  \
   ...
   from \
   ( select mmgg, code_eqp, id_point, name, coalesce(sum(demand),0)::::int as demand, coalesce(sum(losts),0)::::int as losts \
     from bal_grp_tree_conn_tbl where type_eqp=15 and id_voltage not in (4,42) and date_part('year',mmgg::::date) = date_part('year',:dat::::date)  and date_part('month',mmgg::::date) = 6 \
     and id_bal = :res \
     group by code_eqp,id_point,name , mmgg order by id_point) as gt \
    left join bal_demand_tbl as bd  on (bd.id_point=gt.id_point and bd.mmgg = gt.mmgg and bd.id_bal = :res) \
   union \
  ...
  ( gt2.type_eqp=3  or (gt2.type_eqp=8 and gt2.id_voltage <3) ) and date_part('year',gt2.mmgg::::date) = date_part('year',:dat::::date)  and date_part('month',gt2.mmgg::::date) = 6  \
   ...
   from \
   ( select mmgg, code_eqp, id_point, name, coalesce(sum(demand),0)::::int as demand, coalesce(sum(losts),0)::::int as losts \
     from bal_grp_tree_conn_tbl where type_eqp=15 and id_voltage not in (4,42) and date_part('year',mmgg::::date) = date_part('year',:dat::::date)  and date_part('month',mmgg::::date) = 7 \
     and id_bal = :res \
     group by code_eqp,id_point,name, mmgg order by id_point) as gt \
    left join bal_demand_tbl as bd  on (bd.id_point=gt.id_point and bd.mmgg = gt.mmgg and bd.id_bal = :res) \
   union \
  ...
  ( gt2.type_eqp=3  or (gt2.type_eqp=8 and gt2.id_voltage <3) ) and date_part('year',gt2.mmgg::::date) = date_part('year',:dat::::date)  and date_part('month',gt2.mmgg::::date) = 7  \
   ...
   from \
   ( select mmgg, code_eqp, id_point, name, coalesce(sum(demand),0)::::int as demand, coalesce(sum(losts),0)::::int as losts \
     from bal_grp_tree_conn_tbl where type_eqp=15 and id_voltage not in (4,42) and date_part('year',mmgg::::date) = date_part('year',:dat::::date)  and date_part('month',mmgg::::date) = 8 \
     and id_bal = :res \
     group by code_eqp,id_point,name, mmgg order by id_point ) as gt \
    left join bal_demand_tbl as bd  on (bd.id_point=gt.id_point and bd.mmgg = gt.mmgg and bd.id_bal = :res) \
   union \
  ...
  and gt.type_eqp=12 and date_part('year',gt.mmgg::::date) = date_part('year',:dat::::date)  and date_part('month',gt.mmgg::::date) = 8  \
   ...
   from \
   ( select mmgg, code_eqp, id_point, name, coalesce(sum(demand),0)::::int as demand, coalesce(sum(losts),0)::::int as losts \
     from bal_grp_tree_conn_tbl where type_eqp=15 and id_voltage not in (4,42) and date_part('year',mmgg::::date) = date_part('year',:dat::::date)  and date_part('month',mmgg::::date) = 9 \
     and id_bal = :res \
     group by code_eqp,id_point,name, mmgg order by id_point ) as gt \
    left join bal_demand_tbl as bd  on (bd.id_point=gt.id_point and bd.mmgg = gt.mmgg and bd.id_bal = :res) \
   union \
  ...
  and gt.type_eqp=12 and date_part('year',gt.mmgg::::date) = date_part('year',:dat::::date)  and date_part('month',gt.mmgg::::date) = 9  \
   ...
   from \
   ( select mmgg, code_eqp, id_point, name, coalesce(sum(demand),0)::::int as demand, coalesce(sum(losts),0)::::int as losts \
     from bal_grp_tree_conn_tbl where type_eqp=15 and id_voltage not in (4,42) and date_part('year',mmgg::::date) = date_part('year',:dat::::date)  and date_part('month',mmgg::::date) = 10 \
     and id_bal = :res \
     group by code_eqp,id_point,name, mmgg order by id_point ) as gt \
    left join bal_demand_tbl as bd  on (bd.id_point=gt.id_point and bd.mmgg = gt.mmgg and bd.id_bal = :res) \
   union \
  ...
  and gt.type_eqp=12 and date_part('year',gt.mmgg::::date) = date_part('year',:dat::::date)  and date_part('month',gt.mmgg::::date) = 10  \
   ...
   from \
   ( select mmgg, code_eqp, id_point, name, coalesce(sum(demand),0)::::int as demand, coalesce(sum(losts),0)::::int as losts \
     from bal_grp_tree_conn_tbl where type_eqp=15 and id_voltage not in (4,42) and date_part('year',mmgg::::date) = date_part('year',:dat::::date)  and date_part('month',mmgg::::date) = 11 \
     and id_bal = :res \
     group by code_eqp,id_point,name, mmgg order by id_point ) as gt \
    left join bal_demand_tbl as bd  on (bd.id_point=gt.id_point and bd.mmgg = gt.mmgg and bd.id_bal = :res) \
   union \
  ...
  and gt.type_eqp=12 and date_part('year',gt.mmgg::::date) = date_part('year',:dat::::date)  and date_part('month',gt.mmgg::::date) = 11  \
   ...
   from \
   ( select mmgg, code_eqp, id_point, name, coalesce(sum(demand),0)::::int as demand, coalesce(sum(losts),0)::::int as losts \
     from bal_grp_tree_conn_tbl where type_eqp=15 and id_voltage not in (4,42) and date_part('year',mmgg::::date) = date_part('year',:dat::::date)  and date_part('month',mmgg::::date) = 12 \
     and id_bal = :res \
     group by code_eqp,id_point,name, mmgg order by id_point ) as gt \
    left join bal_demand_tbl as bd  on (bd.id_point=gt.id_point and bd.mmgg = gt.mmgg and bd.id_bal = :res) \
   union \
...
// Определим, есть ли вводы на ПС
  ...
  if (selected==0)
  {
   ...
   ( select code_eqp, id_point, name, coalesce(sum(demand),0)::::int as demand, coalesce(sum(losts),0)::::int as losts \
     from  bal_grp_tree_conn_tmp  where type_eqp=15 and id_voltage not in (4,42) and mmgg = :dat group by code_eqp,id_point,name order by id_point) as gt \
    left join bal_demand_tmp as bd    on (bd.id_point=gt.id_point and bd.mmgg = :dat) \
    left join \
...
void __fastcall TfBalansRep::Losts04(TDateTime mmgg)
{
   ...
   ( select code_eqp, id_point, name, demand,demand04,losts,fact_losts,losts_coef,lost04_metod \
     from bal_grp_tree_tmp where type_eqp=15 and id_voltage not in (4,42) and mmgg = :mmgg order by code_eqp) as gt \
