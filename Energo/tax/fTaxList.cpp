...
/*
  ...
  BtnRebuild1->Caption=" Переформ 1 декаду";
...
//  BtnRebuild1->Enabled= !read_only;
  ...
  BtnRebuild2->Caption=" Переформ 2 декаду";
...
*/
  ...
  BtnRebuild3->Caption=" Переформ месяц";
  ...
  BtnCopy->Caption="Копия неплательщику";
...
//  Fieldh->SetReadOnly();
  ...
  Fieldh->AddFixedVariable("8", "Оплата 2016");   
  Fieldh->SetReadOnly();
...
//  Fieldh->SetReadOnly();
  ...
  Fieldh = DBGrCor->AddColumn("tax_date", "Дата НН", "Дата выписки НН");
  Fieldh->SetReadOnly();
...
void TMainForm::TaxDecadeAll(int decade)
{
  ...
  if (MessageDlg(" Сформировать налоговые накладные за "+FormatDateTime("dd.mm.yyyy",mmgg)+"?", mtConfirmation, TMsgDlgButtons() << mbYes << mbNo, 0) != mrYes)
  {
   return;
  }
...
void __fastcall TMainForm::TaxRebuildAbon(int client, int decade,TObject *Sender)
{
  ...
  if (MessageDlg(" Переформировать налоговые накладные по абоненту? \n Налоговые накладные, отмеченные как выданные, переформированы не будут.", mtConfirmation, TMsgDlgButtons() << mbYes << mbNo, 0) != mrYes)
  {
   return;
  }
...
void __fastcall TMainForm::TaxCopyToNoPay(TObject *Sender)
{
  ...
  if (int_num!=0)
  {
     ShowMessage("Невозможно выполнить операцию для налоговой накладной плательщика НДС");
     return;
  }

  if (MessageDlg(" Сформировать налоговую накладную для неплательщиков НДС на основании данной налоговой накладной ?", mtConfirmation, TMsgDlgButtons() << mbYes << mbNo, 0) != mrYes)
  {
   return;
  }
