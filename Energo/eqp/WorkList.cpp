...
//   DBGrWorks->Columns->Items[5]->PickList->Add("Виконати держповірку лічильника електроенергії");
  ...
  Field->Column->PickList->Add("Розрахункові ВК встановити на межі поділу мережі за балансовою належністю");
...
void _fastcall  TfWorkList::OnChangeAct(TWTField *Sender)
{
  ...
  {
    ShowMessage("Заполните номер акта !");
    Table->Cancel();
    return;
  }
...
void __fastcall TfWorkList::WorkBeforePost(TWTDBGrid *Sender)
{
  ...
  {
//    ShowMessage("Заполните номер акта !");
    throw Exception("Заполните номер акта !");
    //qWorkList->Cancel();
    return;
  }
}
