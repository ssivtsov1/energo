...
void _fastcall TMainForm::EqmSwitchBtn(TObject *Sender) {
 ...
 AnsiString fieldnames[]={"Ном.ток,А"};
...
//  NList->Add("id_client");
  ...
  Field = WGrid->AddColumn("cbook", "Книга абон", "Книга");
  Field->SetReadOnly();
  Field = WGrid->AddColumn("ccode", "Счет абон", "Лицевой счет");
  Field->SetReadOnly();
...
#undef WinName
//------------------------------------------------------------
#define WinName "Население для Формы 1 по НДС"
...
*/
  ...
  WGrid->SetCaption("Население для Формы 1 по НДС " +FormatDateTime("dd.mm.yy ",mmgg_sel));
...
*/
  ...
  Field = WGrid->AddColumn("id_section", "Раздел отчета", "Раздел отчета");
  Field->AddFixedVariable("1999","1999 год");
  Field->AddFixedVariable("2000","2000 год");
  Field->AddFixedVariable("2001","до 1 августа 2001");
  Field->AddFixedVariable("2002","до 1 апреля 2011");
  Field->AddFixedVariable("2011","до 1 января 2016");
  Field->SetRequired("Раздел должен быть заполнен");

  Field = WGrid->AddColumn("fixed", "Фикс", "Фикс");
  Field->AddFixedVariable("0","-");
  Field->AddFixedVariable("1","Фикс");

  Field = WGrid->AddColumn("csum", "Контр.", "Контроль");
  Field->SetReadOnly();
  Field = WGrid->AddColumn("csum_pdv", "Контр.НДС", "Контроль НДС");
  Field->SetReadOnly();
  ...
  Field = WGrid->AddColumn("kvt_01", "Потр,кВтч", "Потребление, кВтч");
  Field = WGrid->AddColumn("dem_01", "Потр,грн", "Потребление, грн");
  Field = WGrid->AddColumn("dempdv_01", "Потр НДС,грн", "Потребление НДС, грн");

  Field = WGrid->AddColumn("billkt_01_1", "на погаш.К-т", "в т.ч.на погаш.К-т");
  Field = WGrid->AddColumn("billktpdv_01_1", "на погаш.К-т НДС", "в т.ч.на погаш.К-т НДС");

  Field = WGrid->AddColumn("opl_01", "Оплата", "Оплата");
  Field = WGrid->AddColumn("oplpdv_01", "Оплата НДС", "Оплата НДС");

  Field = WGrid->AddColumn("sp1_01", "Спис.р.п.", "Списано решением правления");
  Field = WGrid->AddColumn("sp1pdv_01", "Спис.р.п.НДС", "Списано решением правления НДС");
...
void __fastcall TMainForm::PeriodSel(TObject *Sender)
{
   ...
   if (rez == mrOk)
   {
     ...
     WF1Grid->SetCaption("Население для Формы 1 по НДС " +FormatDateTime("dd.mm.yy ",mmgg_sel));
   }
};
